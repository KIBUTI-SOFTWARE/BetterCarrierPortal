...
<!-- BEGIN: Content -->
<h2 class="intro-y mt-10 text-lg font-medium">Job Posts (<?= number_format(count($job_applications)) ?>)</h2>
<br>
...
<div class="intro-y mt-5 grid grid-cols-12 gap-6">
    <!-- BEGIN: Blog Layout -->
    <?php
    if (isset($job_applications) && count($job_applications) > 0) {
        foreach ($job_applications as $job_application) {
            $post_id = $job_application['_id'];
            $job_posted_by = (new \App\Controllers\Users)->getUser($job_application['job_post_created_by']);
            ?>
            <div class="intro-y box col-span-12 md:col-span-6 xl:col-span-4">
                    ...
                    <div class="ml-3 mr-auto">
                        ...
                        <div class="mt-0.5 flex truncate text-xs text-slate-500">
                            <a class="inline-block truncate text-primary" href="#">
                                <?= $job_application['job_post_category'] === "1" ? "Employment" : "Internship"; ?>
                            </a>
                            <span class="mx-1">•</span> <?= \Config\MyFunctions::timeAgo($job_application['job_post_created_on']); ?>
                        </div>
                    </div>
                    <?php
                    if ($user_level < "3" || $user['_id'] === $job_application['job_post_created_by']) {
                        ?>
                        <div data-tw-merge="" data-tw-placement="bottom-end" class="dropdown relative ml-3">
                            ...
                            <div data-transition="" data-selector=".show"
                                ...
                                <div data-tw-merge=""
                                     class="dropdown-content rounded-md border-transparent bg-white p-2 shadow-[0px_3px_10px_#00000017] dark:border-transparent dark:bg-darkmode-600 w-40">
                                    <a href="/edit-job-post/<?= $job_application['_id'] ?>"
                                       class="cursor-pointer flex items-center p-2 transition duration-300 ease-in-out rounded-md hover:bg-slate-200/60 dark:bg-darkmode-600 dark:hover:bg-darkmode-400 dropdown-item"><i
                                                data-tw-merge="" data-lucide="edit" class="stroke-1.5 mr-2 h-4 w-4"></i>
                                        Edit Post</a>
                                    <a href="/delete-job-post/<?= $job_application['_id'] ?>"
                                       class="cursor-pointer flex items-center p-2 transition duration-300 ease-in-out rounded-md hover:bg-slate-200/60 dark:bg-darkmode-600 dark:hover:bg-darkmode-400 dropdown-item"><i
                ...
                <div class="p-5">
                    ...
                    <a class="mt-5 block text-base font-medium" href="#">
                        <?= ucwords($job_application['job_post_title']) ?>
                    </a>
                    <div class="mt-2 text-slate-600 dark:text-slate-500">
                        <?= ucfirst($job_application['job_post_title']) ?>
                    </div>
                </div>
